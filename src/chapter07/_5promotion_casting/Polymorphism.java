...
public class Polymorphism {
     ...
     *    => Child 는 Parent 의 확장이고 과정이 어떻던 간에 Child 인스턴스가 Parent 타입에서 제공하지 않는 메서드 및 변수를 가지고 있을 가능성이 존재한다.
     *    => 따라서 parent 는 Child 의 클래스 정보를 가지고 생성된 Parent 타입 인스턴스이며 Parent 타입 뿐만 아니라 Child 타입의 정보도 가지고 동작할 수 있다.
     *    => (중요)여기서 주의 깊게 봐야할 것이 바로 'parent 는 Parent 타입 뿐만 아니라 Child 타입의 정보도 가지고 동작'이다. 이러한 정보 전달로 인해서 다형성을 이용한 객체 지향 프로그래밍이 가능해진다.
     *
     ...
     *    => new Parent()의 정보를 가진 인스턴스는 Child 타입의 정보를 담을 수 없다. Child 클래스에 Parent 에 없는 메서드가 추가 선언되어 있다면 이를 알지 못하는
     *    => Parent 는 정말 당연히 Child 의 기능을 사용할 수 없으므로 자식 클래스의 인스턴스가 슈퍼 클래스의 생성자 정보를 받아서 처리할 수 없는 것이다.
     * */
    ...
    public static void main ( String[] args ) {
        ...
        //이 아래부터 (중요)로 표시한 개념을 코드로 보여주게 됩니다. 'animal 은 Animal 타입 뿐만 아니라 각종 자식 타입의 정보도 가지고 동작' 하게 됩니다.
...
//        Cat cat2 = new Animal ( "없음" ); <- 불가능한 문법으로 컴파일러가 에러를 표시합니다.
        ...
        //cat 도 결국엔 Animal 타입이기 때문에 동일한 부모의 자식 클래스의 메서드를 다형성을 이용해서 활용할 수 있게 됩니다.
        cat.inCage ( animal );
        cat.inCage ( cat );
        cat.inCage ( dog );
        cat.inCage ( fish );

        //inCage 자체는 Animal 에 선언된 메서드이기 때문에 타입이 다른 각각의 자식 클래스도 Animal 타입으로 수용되어 문제없이 동작합니다.
        ...
        //만약 walk 가 Dog 에만 선언된 메서드여도 매개변수의 타입이 부모 타입인 Animal 이라면 Dog 타입 메서드에 Cat 타입 인스턴스가 들어가도 정상적으로 동작합니다.
        huski.walk ( bengol );
    }
}
