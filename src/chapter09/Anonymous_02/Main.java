...
public class Main {
    public static void main ( String[] args ) {
        ...
        /**
         * Version.02 함수형 인터페이스에서 람다식으로
         *
         * 자바 8버전의 주요한 변경점 중 하나가 바로 람다식의 도입이다. 람다식은 함수형 인터페이스의 익명 클래스 형태의 구현이
         * 너무나도 불편하고 코드의 양이 많으며 원하는 바인 화살표 함수 등의 간단한 구현 방식에 부합하도록 만들어졌다.
        ...
        /**
         * 위를 보면 마치 빈 메서드의 선언부만 화살표로 이어져서 실제 동작할 메서드를 가리키는 것과 같다.
         ...
         * 만약 파라미터의 변수가 추론이 불가능하거나 타입 캐스팅을 해야할 상황이 아니라면 파라미터의 타입은 JVM이 추론 가능한 영역이기 때문에 생략해도 무방하다.
         * (보통은 생략하고 변수명만 정해서 씀)
         * */
        ...
        /**
         * 사용 방식은 파라미터와 구현부의 메서드에서 사용하는 파라미터가 동일하다면 JVM이 이를 추론하여
