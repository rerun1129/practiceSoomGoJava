...
public class PureFunction {
     ...
     * 순수 함수 프로그래밍이라 함은 외부에서 들어온 값이나 파라미터를 함수 내부에서 바꾸지 않아야 한다는 것인데
     * 물론 바꾸는 것이 불가능하지는 않지만 외부의 값을 내부에서 변경하는 것은 예기치 못한 사건을
     * 야기시킬 가능성이 있기 때문에 특히 유지보수나 협업시에는 주의해야한다.
     * */

    /**
     * 또한 익명 클래스 내부나 람다식 내부에서는 모든 외부 클래스의 변수가 final이나 유사 final이어야 한다고 하는데 순수 함수 프로그래밍의 특성 상 함수 내부에서는 외부 변수를 변경해서는 안되기 때문에
     * 당연히 외부 변수는 람다식 내부로 값이 들어온 시점을 기준으로 final 취급을 받아야 하고 값이 변경되어서는 안되는 것이다.
     * 의미상 baseNum 을 람다식 내부에서 사용할 때 baseNum 대신에 상수인 10을 써도 의미나 동작이 전혀 달라질 게 없다.
     * */
}
